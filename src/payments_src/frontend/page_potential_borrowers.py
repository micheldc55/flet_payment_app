...
from payments_src.frontend.enums.enums_potential_borrowers import PotentialBorrowersTitle, PagePotentialBorrowersActions
from payments_src.frontend.utils import add_n_line_jumps
from payments_src.shared.pydantic_validation_utils import get_field_input_widget_potential_borrower, get_field_input_widget_car
...
def potential_borrowers_page():
    ...
    selected_action = short_col.selectbox(
        ...
        PagePotentialBorrowersActions.list(),
        index=0,
        key="potential_borrowers_action"
    )

    if selected_action == PagePotentialBorrowersActions.LIST_LOANS.value:
        potential_customers_table = read_potential_customers_filtering_columns(PotentialBorrower.get_displayable_columns())
        ...
        with st.form("add_loan_form", clear_on_submit=True, enter_to_submit=False):
            ...
            form_data = {"borrower": borrower_form_data, "car": car_form_data}
            ...
            if submitted:
                try:
                    ...
                    st.error(f"Error al crear el préstamo: {str(e)}")
        ...
        if approve_loan:
            ...
            potential_customers_table = read_potential_customers_table()
            potential_customers_table = edit_potential_customers_table_record(potential_customers_table, borrower)
            write_potential_customers_table(potential_customers_table, overwrite=True)
        ...
        if reject_loan:
            ...
            write_potential_customers_table(potential_customers_table, overwrite=True)
            st.warning(f"Préstamo Rechazado. ID: {potential_borrower.borrower_id} | Nombre: {potential_borrower.nombre_cliente}")
