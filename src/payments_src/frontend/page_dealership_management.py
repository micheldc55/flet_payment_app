...
def dealership_management_page():
    ...
    selected_action = short_col.selectbox(
        "Selecciona una acción", DealershipManagementActions.list(), index=0, key="dealership_management_action"
    )
    ...
    if selected_action == DealershipManagementActions.ADD_DEALERSHIP.value:
        ...
        if len(dealership_table) == 0:
            ...
            dealership_display_name = short_col.selectbox(
                "Selecciona una automotora", options, index=0, key="dealership_display_name"
            )

            dealership_dict = (
                dealership_table[dealership_table_copy["display_name"] == dealership_display_name].iloc[0].to_dict()
            )
            ...
            with current_values_col:
                ...
                for field_name, field_info in dealership_fields.items():
                    ...
                    st.text_input(
                        f"{field_name} (actual)", value=str(current_value), disabled=True, key=f"current_{field_name}"
                    )
            ...
            with edit_values_col:
                ...
                with st.form("edit_dealership_form", clear_on_submit=False):
                    ...
                    for field_name, field_info in dealership_fields.items():
                        ...
                        field_value = get_field_input_widget_dealership(
                            field_name=field_name, field_info=field_info, key=f"edit_{field_name}"
                        )
                    ...
                    if submitted:
                        try:
                            ...
                            other_dealerships = dealership_table_check[
                                dealership_table_check["dealership_id"] != dealership.dealership_id
                            ]
        ...
        if len(dealership_table) == 0:
            ...
            if delete_dealership:
                dealership_table_to_write = dealership_table[
                    dealership_table_copy["display_name"] != dealership_display_name
                ]
